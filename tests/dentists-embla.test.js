...
async function run() {
  ...
  try {
    ...
    for (const viewport of [
      ...
      const doctorNames = await page.evaluate(
        () => window.SITE_CONFIG?.dentists?.map((d) => d.name) || []
      );
      if (doctorNames.length < 2) {
        throw new Error("Expected at least 2 dentists in SITE_CONFIG for carousel tests");
      }
      const [firstDoctor, secondDoctor] = doctorNames;
      const expectedAfterScroll = (count) => doctorNames[count % doctorNames.length];
      ...
      if (initial.centered !== firstDoctor) {
        throw new Error(`Expected ${firstDoctor} centered initially at ${viewport.width}px, got "${initial.centered}"`);
      }
      if (!initial.leftPeek || !initial.rightPeek) {
        throw new Error(
      ...
      if (secondSnap.centered !== expectedAfterScroll(1)) {
        throw new Error(`Expected ${expectedAfterScroll(1)} after first advance at ${viewport.width}px`);
      }
      ...
      await page.waitForTimeout(700);
      const thirdSnap = await getSlideSnapshot(page);
      if (thirdSnap.centered !== expectedAfterScroll(2)) {
        throw new Error(`Expected ${expectedAfterScroll(2)} after second advance at ${viewport.width}px`);
      }
      if (!thirdSnap.leftPeek || !thirdSnap.rightPeek) {
        throw new Error(
          `Expected neighbors on both sides at third slide (${viewport.width}px, leftPeek=${thirdSnap.leftPeek}, rightPeek=${thirdSnap.rightPeek})`
        );
      }
      ...
      await page.waitForTimeout(700);
      const fourthSnap = await getSlideSnapshot(page);
      if (fourthSnap.centered !== expectedAfterScroll(3)) {
        throw new Error(`Expected ${expectedAfterScroll(3)} after third advance at ${viewport.width}px`);
      }
      if (!fourthSnap.leftPeek || !fourthSnap.rightPeek) {
        throw new Error(
          `Expected neighbors on both sides after third advance (${viewport.width}px, leftPeek=${fourthSnap.leftPeek}, rightPeek=${fourthSnap.rightPeek})`
        );
      }
      ...
      if (!names.includes(firstDoctor) || !names.includes(secondDoctor) || !names.includes(expectedAfterScroll(2))) {
