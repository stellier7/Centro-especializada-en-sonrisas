...
async function assertGalleryLightbox(page) {
  ...
  const galleryImages = await page.evaluate(
    () => window.SITE_CONFIG?.gallery?.filter(Boolean) || []
  );
  if (galleryImages.length < 4) {
    throw new Error("Expected at least 4 gallery images for lightbox tests");
  }
  ...
  if (!imageSrc?.includes(galleryImages[2])) {
    throw new Error(`Expected lightbox to open on ${galleryImages[2]}, got ${imageSrc}`);
  }
  ...
  if (!nextSrc?.includes(galleryImages[3])) {
    throw new Error(`Expected lightbox next image ${galleryImages[3]}, got ${nextSrc}`);
  }

  await page.keyboard.press("ArrowLeft");
  const prevSrc = await lightbox.locator("[data-gallery-lightbox-image]").getAttribute("src");
  if (!prevSrc?.includes(galleryImages[2])) {
    throw new Error(`Expected lightbox previous image ${galleryImages[2]}, got ${prevSrc}`);
  }
...
async function run() {
  ...
  try {
    ...
    for (const viewport of [
      ...
      const expectedSlides = await page.evaluate(
        () => window.SITE_CONFIG?.gallery?.filter(Boolean).length || 0
      );
      if (slideCount !== expectedSlides) {
        throw new Error(`Expected ${expectedSlides} native gallery slides for live config, got ${slideCount}`);
      }
